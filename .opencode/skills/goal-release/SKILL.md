...
name: goal-release
description: >-
  Cut a release end to end: run the draft-release skill to open the release PR
  and draft GitHub release, wait for CI to turn green, run the merge-pr skill to
  merge the release PR, then wait for the Publish workflow to publish the npm
  package and finalize the GitHub release. Use when the user wants to draft and
  merge a release in one go.
---
# Goal Release

new_version = $ARGUMENTS

This skill drives a release all the way to publish. It runs the `draft-release` skill
to open the release pull request (and create the draft GitHub release), waits
for the PR's CI checks to pass, runs the `merge-pr` skill to merge it, and finally waits
for the `Publish` workflow to publish the npm package and flip the GitHub
release public.

## 1. Draft the Release

Run the `draft-release` skill with `new_version`.
...
Run the `merge-pr` skill with the release PR number. It re-verifies the PR
state and CI status, merges with `gh pr merge --admin --merge`, posts a
thank-you comment, and cleans up the local branch.

Note: the `goal-pr` skill tells agents not to auto-merge PRs that touch `package.json` or
